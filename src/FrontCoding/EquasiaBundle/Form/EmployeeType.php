...
class EmployeeType extends AbstractType
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        $currencys = $this->em
            ->getRepository('FrontCodingEquasiaBundle:Currency')
            ->findAll();

        $currency_options = array('' => '--- Please select ---');
        foreach ($currencys as $item) {
            $currency_options[$item->getId()] = $item->getAbbreviation();
        }
        ...
        $builder
            ...
            ->add('idOrPassport')
            ->add('salaryOfficial')
            ->add('salaryUnofficial')
            ->add('salaryOfficialCurrencyId', 'choice', array(
                'choices' => $currency_options,
                'label' => 'Salary official currency',
            ))
            ->add('salaryUnofficialCurrencyId', 'choice', array(
                'choices' => $currency_options,
                'label' => 'Salary unofficial currency',
            ))
            ->add('salaryNotes')
    ...
    public function setDefaultOptions(OptionsResolverInterface $resolver)
    {
        $resolver->setDefaults(array(
            'data_class' => 'FrontCoding\EquasiaBundle\Entity\Employee'
        ));
    }
    ...
    {
        return 'frontcoding_equasiabundle_employee';
    }
}
